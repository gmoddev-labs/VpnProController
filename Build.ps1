param([switch]$Installer, [string]$IsccPath, [string]$Version = '0.2.0')
...
$AppOutput = Join-Path $ProjectRoot $(if ($Installer) { 'dist\App' } else { 'App' })
$PublishOptions = @('-c','Release','-m:2','-p:UseSharedCompilation=false',"-p:Version=$Version")
if ($Installer) { $PublishOptions += @('--self-contained','true','-p:WindowsAppSDKSelfContained=true') }
...
if ($Installer) { $RecoveryOptions += @('--self-contained','true') }
